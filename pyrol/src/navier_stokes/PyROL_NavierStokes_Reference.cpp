...
std::string rankPath(const std::string &path, const int rank) {
  std::string out = path;
...
std::vector<RealT> readVectorFile(const std::string &path) {
...
class NavierStokesReference {
public:
  NavierStokesReference(const std::string &xmlPath,
    ...
    nt_ = parlist_->sublist("Time Discretization")
              .get("Number of Time Steps", 100);
    if (nt_ < 2) {
      throw std::invalid_argument("Number of Time Steps must be at least 2");
    }
    const RealT T =
        parlist_->sublist("Time Discretization").get("End Time", 1.0);
    ...
    if (!useParametricControl) {
      throw std::invalid_argument(
          "NavierStokesReference supports only parametric control");
    }
  ...
  int localSize() const { return nt_; }

  int evaluateAndPrint(const std::vector<RealT> &zValues,
                       const std::vector<RealT> &vValues,
                       const RealT tol = 1e-8) {
    if (static_cast<int>(zValues.size()) != nt_) {
      std::ostringstream os;
      os << "expected " << nt_ << " z entries, got " << zValues.size();
      throw std::runtime_error(os.str());
    }
    if (static_cast<int>(vValues.size()) != nt_) {
      std::ostringstream os;
      os << "expected " << nt_ << " v entries, got " << vValues.size();
      throw std::runtime_error(os.str());
    }

    copyToPartitionedVector(*z_, zValues);
    copyToPartitionedVector(*v_, vValues);

    auto timed = [this](const std::function<void()> &op) {
      comm_->barrier();
    ...
    const RealT valueSeconds = timed([&]() {
      ...
      value = objective_->value(*z_, valueTol);
    });
    RealT gradientTol = tol;
    const RealT gradientSeconds = timed([&]() {
      objective_->update(*z_, true, -1);
      g_->zero();
      objective_->gradient(*g_, *z_, gradientTol);
    });
    RealT hessVecTol = tol;
    const RealT hessVecSeconds = timed([&]() {
      objective_->update(*z_, true, -1);
      hv_->zero();
      objective_->hessVec(*hv_, *v_, *z_, hessVecTol);
    });

    const std::vector<RealT> gradient = copyFromPartitionedVector(*g_);
    const std::vector<RealT> hessVec = copyFromPartitionedVector(*hv_);
    const RealT gradientDot = g_->dot(*v_);
    const RealT hessVecDot = hv_->dot(*v_);

    std::cout << std::setprecision(17);
    std::cout << "RESULT " << rank_ << " " << localSize() << " "
              << value << " " << gradientDot << " " << hessVecDot << " "
              << valueSeconds << " " << gradientSeconds << " "
              << hessVecSeconds << " " << gradient.size();
...
private:
  void patchParameterList() {
    ...
    parlist_->sublist("Dynamic Constraint")
        .sublist("Solve")
        .set("Output Iteration History", false);
    parlist_->sublist("SimOpt")
        .sublist("Solve")
        .set("Output Iteration History", false);
  ...
  void buildObjective() {
    ...
    dynCon_ = ROL::makePtr<DynConstraint<RealT>>(
        pde_, meshMgr_, comm_, *parlist_, *outStream_);
    ...
    ROL::Ptr<Tpetra::MultiVector<>> uoPtr =
        assembler_->createStateVector();
    ROL::Ptr<Tpetra::MultiVector<>> unPtr =
        assembler_->createStateVector();
    ROL::Ptr<Tpetra::MultiVector<>> ckPtr =
        assembler_->createResidualVector();

    u0_ = ROL::makePtr<PDE_PrimalSimVector<RealT>>(
        u0Ptr_, pde_, *assembler_, *parlist_);
    uo_ = ROL::makePtr<PDE_PrimalSimVector<RealT>>(
        uoPtr, pde_, *assembler_, *parlist_);
    un_ = ROL::makePtr<PDE_PrimalSimVector<RealT>>(
        unPtr, pde_, *assembler_, *parlist_);
    ck_ = ROL::makePtr<PDE_DualSimVector<RealT>>(
        ckPtr, pde_, *assembler_, *parlist_);

    zk_ = ROL::makePtr<PDE_OptVector<RealT>>(
        ROL::makePtr<ROL::StdVector<RealT>>(1));
    z_ = ROL::PartitionedVector<RealT>::create(*zk_, nt_);
    g_ = ROL::dynamicPtrCast<ROL::PartitionedVector<RealT>>(
        z_->dual().clone());
    v_ = ROL::dynamicPtrCast<ROL::PartitionedVector<RealT>>(z_->clone());
    hv_ = ROL::dynamicPtrCast<ROL::PartitionedVector<RealT>>(
        z_->dual().clone());
    ...
    const RealT w2 =
        parlist_->sublist("Problem").get("State Boundary Cost", 1.0);
    const RealT w3 = parlist_->sublist("Problem").get("Control Cost", 0.0);
    const RealT wT =
        parlist_->sublist("Problem").get("Final Time State Cost", 1.0);
    std::vector<RealT> weights = {w1, w2, w3};
    std::vector<RealT> terminalWeights = {wT};

    const std::string integratedObjective =
        parlist_->sublist("Problem")
            .get("Integrated Objective Type", "Dissipation");
    const std::string finalObjective =
        parlist_->sublist("Problem")
            .get("Final Time Objective Type", "Tracking");

    qoiVec[0] = ROL::makePtr<QoI_State_NavierStokes<RealT>>(
        integratedObjective, *parlist_, pde_->getVelocityFE(),
        pde_->getPressureFE(), pde_->getFieldHelper());
    qoiVec[1] =
        ROL::makePtr<QoI_DownStreamPower_NavierStokes<RealT>>(
            pde_->getVelocityFE(), pde_->getPressureFE(),
            pde_->getVelocityBdryFE(1), pde_->getBdryCellLocIds(1),
            pde_->getFieldHelper());
    qoiVec[2] = ROL::makePtr<QoI_RotationControl_NavierStokes<RealT>>();
    qoiTerminal[0] = ROL::makePtr<QoI_State_NavierStokes<RealT>>(
        finalObjective, *parlist_, pde_->getVelocityFE(),
        pde_->getPressureFE(), pde_->getFieldHelper());
    ...
    ROL::Ptr<ROL::Objective_SimOpt<RealT>> objT =
        ROL::makePtr<PDE_Objective<RealT>>(
            qoiTerminal, terminalWeights, assembler_);
  ...
  void initializeState() {
    ...
    name << "initial_condition_Re" << static_cast<int>(reynoldsNumber)
         << ".txt";
    ...
    PotentialFlow<RealT> potentialFlow(
        pde_->getVelocityFE(), pde_->getPressureFE(), pde_->getCellNodes(),
        assembler_->getDofManager()->getCellDofs(), assembler_->getCellIds(),
        pde_->getFieldHelper(), *parlist_);
  ...
  void copyToPartitionedVector(ROL::PartitionedVector<RealT> &target,
    ...
    if (target.numVectors() !=
        static_cast<ROL::PartitionedVector<RealT>::size_type>(nt_)) {
      throw std::runtime_error(
          "internal ROL vector has an unexpected number of partitions");
    }
    for (int k = 0; k < nt_; ++k) {
      ...
      if (zk == ROL::nullPtr || zk->getParameter() == ROL::nullPtr) {
        throw std::runtime_error(
            "internal ROL vector is not a parametric control vector");
      }
      ROL::Ptr<std::vector<RealT>> parameter =
          zk->getParameter()->getVector();
      if (parameter->size() != 1) {
        throw std::runtime_error(
            "expected exactly one scalar parameter per time step");
      }
  ...
  std::vector<RealT>
  copyFromPartitionedVector(ROL::PartitionedVector<RealT> &source) const {
    std::vector<RealT> values(nt_, 0);
    if (source.numVectors() !=
        static_cast<ROL::PartitionedVector<RealT>::size_type>(nt_)) {
      throw std::runtime_error(
          "internal ROL vector has an unexpected number of partitions");
    }
  ...
  try {
    if (argc != 5) {
      std::cerr << "usage: " << argv[0]
                << " input.xml cache_dir z.txt v.txt\n";
      return 2;
    }

    ROL::Ptr<const Teuchos::Comm<int>> comm = Tpetra::getDefaultComm();
    const int rank = comm->getRank();
    NavierStokesReference objective(argv[1], argv[2], 0.0);
    const std::vector<RealT> zValues = readVectorFile(rankPath(argv[3], rank));
    const std::vector<RealT> vValues = readVectorFile(rankPath(argv[4], rank));
    return objective.evaluateAndPrint(zValues, vValues);
